...
- ==Data is delivered progressively(dần dần), not all at once.==
...
- `media_type="text/event-stream"` - đây là Content-Type đặc trưng của SSE
- Format `data: Message {i}\n\n` - đúng format SSE
- **StreamingResponse** là một class trong FastAPI cho phép server gửi data từng phần (chunks) thay vì gửi toàn bộ cùng lúc.
### Python
...
The differences between how streaming works in a =="classical" manner== ==versus== how it works in ==MCP== can be depicted(mô tả) like so:
...
- **For ==simple streaming needs==:** Classic HTTP streaming is simpler to implement and sufficient(đủ) for basic streaming needs.
...
Understanding how streaming works within the MCP framework is essential for building ==responsive applications== tha==t provide real-time feedback to users during long-running operations.==

In ==MCP==, ==streaming== is ==not== about ==sending the main response in chunks==, ==but== about ==sending **notifications** to the client== ==while== a ==tool== is ==processing a request==. These ==notifications== ca==n include progress updates, logs, or other events.==
...
For example, ==a client is supposed to(được yêu cầu) send a notification once the initial handshake with the server has been made.==
...
This section explains the concept of progress notifications in MCP, why they matter, and how to implement them using Streamable HTTP. You'll also find a practical assignment to reinforce(củng cố) your understanding.
...
There are ==two compelling (thuyết phục) reasons== to upgrade from SSE to Streamable HTTP:
...
[[mcp/mcp for beginners microsoft/Building First Server & Client/06-http-streaming/solution/solution|solution]]
