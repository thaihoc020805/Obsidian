# Consuming(sử dụng) a server from GitHub Copilot Agent mode
...
- ==Terminal==, it's possible to control things from the terminal using the `code` exectuable:
  ...
  To a==dd an MCP server to your user profile==, use the ==--add-mcp== command line option, and provide the JSON server configuration in the form =={\"name\":\"server-name\",\"command\":...}.==
...
![[chat-mode-agent.png]]
![[agent-mode-select-tools.png]]
![[mcp-list-servers.png]]
...
Start by creating a config file in your project root, you will need a file called MCP.json and to place it in a folder called .vvvsscode. It should look like so:
...
1. Locate your entry in *mcp.json* and make sure you find the "play" icon:
  ...
  ![[vscode-start-server.png]]

2. Click the "play" icon, you should see the tools icon in the GitHub Copilot Chat increase the number of available tools. If you click said tools icon, you will see a list of registered tools. You can check/uncheck each tool depending if you want GitHub Copilot to use them as context:

  ![[vscode-tool.png]]

3. To run a tool, type a prompt that you know will match the description of one of your tools, for example a prompt like so "add 22 to 1":
...
    "servers": {
        "hello-mcp": {
            "command": "D:/Workspace/fci/mcp/microsoft_mcp_beginner/calculator-server/venv/Scripts/python.exe",
            "args": [
                "server.py"
            ]
