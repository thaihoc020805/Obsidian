...
In this lesson, we will explore ==how to configure sampling parameters in MCP requests== and u==nderstand the underlying protocol mechanics of sampling.==
...
- ==Implement deterministic sampling for reproducible(có thể tái tạo) results.==
...
| -------------------------- | ---------------------------------------------------------------------------------- | ------------- |
| `top_p`                    | Nucleus sampling - limits tokens to top cumulative(tích luỹ) probability(xác xuất) | 0.0 - 1.0     |
| `top_k`                    | Limits token selection to top K options                                            | 1 - 100       |
| `presence_penalty`         | Penalizes tokens based on their presence(sự xuất hiện) in the text so far          | -2.0 - 2.0    |
...
MCP sampling is designed with ==human oversight==(sự giám sát) in mind:
...
- **==Sanitize(làm sạch) sensitive information==** from ==prompts== and ==completions==
...
Sampling ==parameters== allow ==fine-tuning==(tinh chỉnh) ==the behavior== of language models to achieve the desired balance between ==deterministic== and ==creative== outputs.
...
- Used:
    ...
    - `top_k` to restrict the model to the top K most probable tokens, which can help in generating more coherent(mạch lạch) responses.
...
- Defined sampling presets fo==r different task types== (creative, factual, code, analytical).
