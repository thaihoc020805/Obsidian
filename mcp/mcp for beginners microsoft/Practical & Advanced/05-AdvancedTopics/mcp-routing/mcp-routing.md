...
- Handled cases where the p==rompt does not match any specialized category by routing to a general service.==

connect đến đúng client mà nó phù hợp với context
...
- ==**Round Robin**==: Distributes requests evenly(đều) across available servers.
...
client nó kết nối đến nhiều server
...
- Implemented ==versioning support for tools==, allowing users to spe==cify which version of a tool they want to use.==
...
[[mcp/mcp for beginners microsoft/Practical & Advanced/05-AdvancedTopics/mcp-routing/graph.canvas|graph]]
## What's next
