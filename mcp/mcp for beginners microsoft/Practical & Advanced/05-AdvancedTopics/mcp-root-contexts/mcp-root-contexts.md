...
Root contexts are a fundamental concept in the Model Context Protocol that provide a p==ersistent layer for maintaining conversation history== and ==shared state across multiple requests and sessions==.
...
- ==**Conversation Persistence**:== Maintaining coherent(mạch lạc) multi-turn conversations
...
- ==They can contain conversation history, user preferences(sở thích), and other metadata==.
...
4. ==Retrieved context information to understand the conversation history==.
5. ==Archived== the context ==when== the ==conversation was complete.==

lưu ý ở mỗi lần response trả về thì root context bản thân nó đã lưu trữ cả prompt lần response
## Example: Root Context Implementation for f==inancial analysis==
...
2. ==Sent multiple messages within that context, allowing the model to maintain state.==
3. ==Updated== the context ==with relevant metadat==a based on the conversation.
...
2. ==Sent multiple messages within that context==, allowing the model to maintain state with the function `sendMessage`. The messages being sent are about slow query performance and index configuration.
...
2. ==Sent multiple messages within that context==, allowing the model to ==maintain state== with the function `send_message`. The messages being sent are about issues with the auto-scaling feature.
...
- ==**Set Expiration Policies**==: Implement ==policies to archive or delete old contexts to manage storage and comply(tuân thủ) with data retention policies.==
